...
public class JacksonEngine implements Unmarshaller, Marshaller {
    ...
    public BsonDocument  marshall(Object pojo) throws MarshallingException {
        ...
        // Log or print the byte array content for debugging
        System.out.println("Byte Array Content: " + Arrays.toString(byteArray));

        // Attempt to create a BSON document from the byte array
        try {
//            org.bson.BsonDocument bsonDocument = org.bson.BsonDocument.parse(jsonString);
            BsonDocument document = Bson.createDocument(byteArray);
            return document;
        } catch (JsonParseException e) {
            throw new MarshallingException("Failed to parse BSON from byte array: " + Arrays.toString(byteArray), e);
        }
