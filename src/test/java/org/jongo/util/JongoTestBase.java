...
import org.jongo.MongoCollectionWrapper;
...
public abstract class JongoTestBase {
  ...
  protected MongoCollectionWrapper createEmptyCollection(String collectionName) {
    MongoCollectionWrapper col = jongo.getCollection(collectionName);
    col.drop();
    return col;
  }
  ...
  protected void assumeThatMongoVersionIsGreaterThan(String expectedVersion)
      throws UnknownHostException {
    int expectedVersionAsInt = Integer.valueOf(expectedVersion.replaceAll("\\.", ""));
    Document buildInfo = getDatabase().runCommand(new Document("buildInfo", 1));
