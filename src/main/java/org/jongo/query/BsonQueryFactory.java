...
public class BsonQueryFactory implements QueryFactory {
   ...
   * {"firstname":#} -> {"firstname":{MARSHALL_OPERATOR: 0}} 0 being the index of the parameter to
   * be inserted in place of that placeholder. Previously $marshall but upgrading to mongo driver 4
   * the new parser does not allow $ prefixed strings if they're not mongo operators. With a UUID
   * prefixed string there should be no risk of collision.
   */
  ...
  private static class BsonQuery implements Query {
    ...
    public org.bson.BsonDocument toBsonDocument() {
      return BsonDocumentWrapper.asBsonDocument(dbo, MongoClientSettings.getDefaultCodecRegistry());
    }
  }
  ...
  private String addRequiredQuotesAndParameters(String query, Object[] parameters) {
    ...
    for (char nextChar : query.toCharArray()) {
      if (ctxStack.peek() == Context.STRING) {
        ...
        previousToken = specialChar(nextChar).applySpecificBehaviour(result, currentToken, ctxStack,
            position);
      } else {
        currentToken.append(nextChar);
      }
  ...
  private Object marshallParameterAsPrimitive(Object parameter) {
    ...
    return document.toDocument().get("wrapped");
  }
}
